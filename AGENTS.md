...
## Releases
- Keep release metadata, changelogs, and relevant documentation synchronized with every release.
- Update `.github/release-trigger` only when intentionally publishing a release.
...
- Keep Semgrep, Gitleaks, CodeQL, Android Lint, Detekt, and the build/test suite fail-closed where they are part of the required verification path. Use Dependency Review as a required gate only when GitHub Dependency Graph support is enabled and stable for this repository. Qodana may remain a scheduled/manual defense-in-depth scan rather than a flaky merge gate.
