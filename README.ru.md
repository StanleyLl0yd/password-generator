...
- Раздел «О приложении» с описанием, установленной версией, автором, лицензией, Политикой конфиденциальности и ссылкой на официальный сайт приложения
...
- Захват экрана блокируется, пока сгенерированный пароль явно раскрыт пользователем; обычный интерфейс со скрытым паролем остаётся доступным для скриншотов
- Состояния и результаты, содержащие пароль, маскируют его в своих строковых представлениях
...
Ссылки из раздела «О приложении» открываются Android во внешнем приложении, например браузере. Полный текст приведён в [PRIVACY.ru.md](PRIVACY.ru.md).
...
- Semgrep
- Gitleaks
- целостность Gradle Wrapper и CI supply chain

Qodana запускается по расписанию или вручную как дополнительный слой анализа и не является внешним merge-blocking gate.
